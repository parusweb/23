...
function display_product_calculators() {
    ...
    switch ($calculator_type) {
        ...
        case 'running_meter':
            display_running_meter_calculator_new($product_id, $price_with_multiplier);
            break;
...
function display_square_meter_calculator($product_id, $price, $area_data) {
    ...
    jQuery(document).ready(function($) {
        $('#custom_sq_width, #custom_sq_length').on('input', function() {
            ...
            $('#sq_total_price').text(Math.round(totalPrice).toLocaleString('ru-RU') + ' ₽');
...
/**
 * Калькулятор размеров (для товаров с площадью в названии)
 */
function display_dimensions_calculator($product_id, $price, $area_data) {
    ...
    jQuery(document).ready(function($) {
        $('#custom_mult_width, #custom_mult_length').on('input', function() {
            ...
            $('#mult_total_price').text(Math.round(totalPrice).toLocaleString('ru-RU') + ' ₽');
...
/**
 * Калькулятор погонных метров (НОВАЯ ВЕРСИЯ)
 */
function display_running_meter_calculator_new($product_id, $price) {
    ?>
    ...
    jQuery(document).ready(function($) {
        $('#custom_rm_length').on('input', function() {
            ...
            const totalPrice = length * basePrice;
            
            $('#rm_total_length').text(length.toFixed(2));
            $('#rm_total_price').text(Math.round(totalPrice).toLocaleString('ru-RU') + ' ₽');
...
function display_falsebalk_calculator($product_id, $price) {
    ...
    jQuery(document).ready(function($) {
        ...
        $('#falsebalk_width, #falsebalk_height, #falsebalk_length').on('change input', function() {
            ...
            $('#falsebalk_total_price').text(Math.round(totalPrice).toLocaleString('ru-RU') + ' ₽');
            $('#custom_rm_total_price').val(totalPrice);
...
function display_area_calculator() {
    // Функция перенесена в display_product_calculators()
